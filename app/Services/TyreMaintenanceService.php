...
class TyreMaintenanceService
{
    ...
    public function reject(TyreMaintenance $maintenance, ?string $reason = null): TyreMaintenance
    {
        ...
        return DB::transaction(function () use ($maintenance, $reason): TyreMaintenance {
    ...
    {
        return DB::transaction(function () use ($data, $preparedBy): TyreMaintenance {
            ...
            $this->assertNoOpenMaintenance($tyre);

            $maintenance = TyreMaintenance::query()->create(array_merge($data, [
    ...
    protected function assertNoOpenMaintenance(Tyre $tyre): void
    {
        $hasOpenMaintenance = TyreMaintenance::query()
        ...
        if ($hasOpenMaintenance) {
            throw new TyreBusinessException('Tyre has an open maintenance record.');
        }
    }

    protected function restoreTyreStatus(Tyre $tyre): void
    {
        if ($tyre->isDisposed()) {
            return;
        }
