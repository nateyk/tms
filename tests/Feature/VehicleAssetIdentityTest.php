...
class VehicleAssetIdentityTest extends TestCase
{
    ...
    public function test_vehicle_display_does_not_duplicate_same_code_and_plate(): void
    {
        $vehicle = Vehicle::query()->create([
            'vehicle_code' => ' et-3-a20002 ',
            'plate_number' => ' et-3-a20002 ',
            'asset_type' => AssetType::PowerVehicle,
            'vehicle_type_id' => $this->powerType->id,
            'status' => VehicleStatus::Active,
        ]);

        $this->assertTrue($vehicle->hasSameCodeAndPlate());
        $this->assertSame('ET-3-A20002', $vehicle->displayCodeWithPlate());
        $this->assertSame('Same as plate', $vehicle->vehicleCodeDisplay());
    }

    public function test_vehicle_display_keeps_different_code_and_plate(): void
    {
        $vehicle = Vehicle::query()->create([
            'vehicle_code' => 'TRK-DISPLAY-001',
            'plate_number' => 'AA-DISPLAY-001',
            'asset_type' => AssetType::PowerVehicle,
            'vehicle_type_id' => $this->powerType->id,
            'status' => VehicleStatus::Active,
        ]);

        $this->assertFalse($vehicle->hasSameCodeAndPlate());
        $this->assertSame('TRK-DISPLAY-001 / AA-DISPLAY-001', $vehicle->displayCodeWithPlate());
        $this->assertSame('TRK-DISPLAY-001', $vehicle->vehicleCodeDisplay());
    }

    public function test_vehicle_detail_page_renders_for_authenticated_user(): void
    {
